...
- The Agent queue head now covers every trigger source, not only A2A-to-A2A delivery. Ordinary schedules, manual/system work, and A2A messages for the same Agent are claimed in durable creation order across daemon ticks and workers; a partial PostgreSQL uniqueness contract rejects a second active execution, while abandoned work for a disabled trigger is retired before the next claim.
...
- RC5 closes the final independent-review blockers with 1,058 backend tests and 89 frontend tests, a 7,001-module production build, and 85 focused authorization/trigger regressions. The PostgreSQL fresh-install and downgrade/re-upgrade gate now also proves cross-source per-Agent queue ordering and the new owner/session uniqueness constraints. The Alembic revision contract, 148 deployment/storage/migration tests, Ruff Git-baseline gate across 20 changed Python files, Bash syntax, effective production Compose, TypeScript compilation, and `git diff --check` pass.
