...
- Code execution is a separately authorized high-risk capability, not a model permission. It is disabled by default and requires all three gates: the platform kill switch, an exact tenant UUID allowlist, and an explicit enabled assignment for the exact Agent. Wildcards are rejected and every gate is rechecked immediately before execution.
...
- Production Code accepts only explicitly approved external isolation backends. Local `subprocess`/`docker`, unsafe bubblewrap fallback, and implicit fallback to the host are rejected. The production API and worker no longer mount the Docker socket or run with `privileged`, `SYS_ADMIN`, or unconfined seccomp/AppArmor settings.
- Migration `095_secure_code_execution_defaults.py` changes every Code tool to non-default, disables all historical Agent Code assignments, and resets network/unsafe-fallback values at Tool, company, and Agent override levels. It also formalizes the previously runtime-created `tenant_settings` table for migration-only deployments. A deployment never enables Code automatically; a separate operator authorization and external-sandbox preflight are required after release.
...
- RC2 local release gates passed with 862 backend tests, 60 frontend tests, a 6,995-module production frontend build, and the complete PostgreSQL migration/rollback/re-upgrade smoke covering Credits settlement, production issue aggregation, media-generation exactly-once behavior, A2A queue serialization, preference/queue concurrency contracts, and fail-closed Code grant migration. The focused deployment/security gate passed 327 cases, including Nginx `map` parsing, full-length Worker container identity, tool tenant/secret boundaries, Code authorization and approval integrity, sandbox policy, and the exactly-one healthy Worker boundary. Ruff passed for the new/modified Python modules outside the legacy `agent_tools.py` lint baseline, and the critical Ruff rules plus Python compile passed for that dispatcher; Bash syntax, development/production Compose parsing, and `git diff --check` also passed. Independent review, production cutover, browser business-flow proof, and post-release observation remain separate gates.
