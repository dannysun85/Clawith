...
- Slot journals and the `current` symlink now resolve only to a complete ASCII-named direct child of the managed `releases/` directory. Nested paths, Unicode control characters, symlinked release metadata, missing or non-regular metadata, and malformed journals fail closed before recovery, cleanup, or traffic mutation.
...
- The current production-closure working tree has passed all 1,525 backend tests, all 100 frontend tests, the two bounded browser assertions and the 7,002-module production build. The complete PostgreSQL fresh-install, production-era upgrade, blocker, downgrade and re-upgrade smoke; RC4 Ruff baseline gate; Python compilation; Alembic single-head check; Bash, diff and effective-Compose gates also pass. These remain working-tree results until the branch is frozen and the exact-commit gates complete.
- Until those exact-commit gates complete, the tree is **not yet a `local_rc_candidate` or `business_flow_proven` build**. It is not merged, tagged, deployed, provider-verified or production-verified. Historical RC3/RC4 evidence above remains evidence for those earlier candidates, not proof for this changed tree.
