...
- Every ordinary production deployment now actively rewrites the Code platform switch to false and empties the tenant, tool, provider, and endpoint allowlists instead of inheriting a prior release's `.env`. **Code activation is not part of v1.10.12**: it remains blocked until informed approval preview, crash-safe non-replay execution state, provider dependency/contract smoke, hard timeout/output/concurrency controls, Credits reservation/settlement, and dedicated production metrics/alerts are complete and separately reviewed.
...
- RC2 local release gates passed with 876 backend tests, 60 frontend tests, a 6,995-module production frontend build, and the complete PostgreSQL migration/rollback/re-upgrade smoke covering Credits settlement, production issue aggregation, media-generation exactly-once behavior, A2A queue serialization, preference/queue concurrency contracts, and fail-closed Code grant migration. The focused deployment/security gate passed 341 cases, including Nginx `map` parsing, full-length Worker container identity, tool tenant/secret/MCP runtime boundaries, deterministic tenant tool naming, Code authorization and approval integrity, sandbox policy, and the exactly-one healthy Worker boundary. Ruff passed for the new/modified Python modules outside the legacy `agent_tools.py` lint baseline, and the critical Ruff rules plus Python compile passed for that dispatcher; Bash syntax, development/production Compose parsing, Alembic single-head verification, and `git diff --check` also passed. Independent review, production cutover, browser business-flow proof, and post-release observation remain separate gates.
