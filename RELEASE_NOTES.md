...
- WeCom Webhook configuration, ordinary Webhook replies, and proactive delivery now share one positive ASCII-numeric `wecom_agent_id` contract. Invalid legacy rows fail before session creation or LLM/Credits work, and every ordinary outbound message serializes `agentid` as an integer instead of silently sending application ID `0`.
