...
- Proactive WeCom delivery now requires a positive numeric `wecom_agent_id`, validates it before any provider request, and serializes the outbound `agentid` as an integer instead of silently relying on an invalid default application ID. Missing or malformed configuration fails closed with an actionable operator error.
...
- RC4 extends the same local release gates to 1,013 backend tests and 81 frontend tests with the 6,999-module production build. New adversarial coverage verifies `2062` deterministic rejection settlement, independent-credential cooldown selection, unknown error-text sanitization, Redis read/write interruption fallback, routed metadata forwarding across every external channel, WeChat tool-call history conversion, positive-integer WeCom application-ID enforcement, and the text-only channel capability contract across the runtime prompt, live tool schema, database seed, callback registration, and provider-confirmed delivery result. The complete PostgreSQL fresh-install and production-era upgrade/downgrade/re-upgrade smoke, Alembic single head, Ruff Git-baseline gate, Bash syntax, effective production Compose rendering, and `git diff --check` pass.
