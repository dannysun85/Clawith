...
- Password/SSO registration and unverified login now resolve one authoritative SMTP verification-policy snapshot. A configuration lookup failure returns `503` instead of being confused with a confirmed no-SMTP installation, and the same snapshot is passed through provider exchange, Identity and User creation so a transient second read cannot activate an account under an unknown policy.
...
- RC5 adds cross-source per-Agent queue ordering, owner/session uniqueness, authenticated recovery smoke, media compensation assertions, release/config-bound alert canaries, exact Worker delivery attribution, fail-closed Password/SSO verification-policy resolution, and a real Nginx container boot to the release gates. The exact local candidate passes 1,389 backend tests and 94 frontend tests across 22 files, the 7,001-module production frontend build, 143 production-deployment/canary contracts, and the complete PostgreSQL fresh-install plus production-era upgrade/downgrade/re-upgrade smoke through `alert_worker_attribution`. Ruff, Python compilation, Bash syntax, effective production Compose rendering, Alembic single-head/revision checks, `git diff --check`, isolated credential-bound API smoke, and the real-browser product flow also pass on that candidate.
...
- This evidence makes the tree a **local Code-off RC5 candidate only**. Production cutover, production resolver/endpoint checks, production data-plane proof, and post-release observation remain separate gates and have not been claimed by this local validation.
...
- After the cutover, operators must rotate provider-side bot tokens, signing/encryption keys, and verification tokens, save the replacements through Astra, verify inbound/outbound channel traffic, revoke the previous values, and apply the approved WAL/backup retention policy. Until that provider rotation and retention window are complete, historical-secret exposure remains an operational residual risk rather than a fully closed code defect.
- The production deploy is forward-only after the durable approval schema gate. Returning traffic to an application version that does not understand the encrypted `ChannelConfig` contract is prohibited.
