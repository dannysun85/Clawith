...
- RC5 adds cross-source per-Agent queue ordering, owner/session uniqueness, authenticated recovery smoke, media compensation assertions, release/config-bound alert canaries, and exact Worker delivery attribution to the release gates. The integrated tree passes 1,378 backend tests and 93 frontend tests across 21 files, the 7,001-module production frontend build, 143 production-deployment/canary contracts, and the complete PostgreSQL fresh-install plus production-era upgrade/downgrade/re-upgrade smoke through `alert_worker_attribution`. Ruff, Python compilation, Bash syntax, effective production Compose rendering, Alembic single-head/revision checks, and `git diff --check` also pass. These gates are rerun once more against the exact clean release commit before deployment.
