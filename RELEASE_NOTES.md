...
- Media capability discovery now applies the same platform-pool boundary as runtime credential selection. A tenant-private MiniMax credential can no longer make a shared SaaS image/audio/music/video capability appear available to other companies.
...
- The complete backend suite passes: **715 tests**.
...
- Rollback stops the candidate API/worker and the legacy API writer before restoring the exact pre-release Alembic revision. A failed database downgrade leaves both API writers and workers quiesced for operator recovery from the captured database backup; the rollback trap always exits non-zero and cannot continue into a false deployment success.
