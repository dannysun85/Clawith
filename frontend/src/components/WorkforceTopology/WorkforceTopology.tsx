                    ...
                    <section className="workforce-drawer__section">
                        ...
                        {work ? (
                            <button
                                ...
                                onClick={() => navigate(work.deep_link)}
                            >
                                <span className="workforce-work-chip" data-work={work.stage}>
                                    {workGroupLabel(work.stage, t)}
                                </span>
                ...
                <footer className="workforce-drawer__footer">
                    ...
                    {node.can_manage && (
                        <button
                            ...
                            onClick={() => navigate(`/agents/${node.id}/settings#settings`)}
                        >
...
export default function WorkforceTopology({ topology }: WorkforceTopologyProps) {
    const { t } = useTranslation();
    ...
    const workByAgent = useMemo(
        () => new Map(
            topology.nodes.flatMap((node) => (
                node.work ? [[node.id, node.work] as const] : []
            )),
        ),
        [topology.nodes],
    );
    const visibleNodes = useMemo(() => filterTopologyNodes(topology.nodes, {
        ...
        health: healthFilter,
        work: workFilter,
    }), [healthFilter, query, topology.nodes, workFilter]);
    const nodeIdKey = topologyNodeIdKey(visibleNodes);
