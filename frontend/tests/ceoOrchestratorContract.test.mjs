...
test('CEO brief panel is invisible unless this Agent is the enabled CEO', () => {
  assert.match(briefPanel, /settings\?\.feature_available/);
  assert.match(briefPanel, /settings\.configured/);
  assert.match(briefPanel, /settings\.enabled/);
  assert.match(briefPanel, /settings\.ceo_agent_id === agentId/);
