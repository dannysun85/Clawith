...
async def main() -> None:
    ...
    async with async_session() as db:
        ...
        legacy_task = await db.scalar(
            select(MediaGenerationTask).where(MediaGenerationTask.reservation_id == legacy_reservation_id)
        )
    ...
    storage.objects[f"{agent_id}/workspace/videos/legacy-finalized.mp4"] = video_bytes
    ...
    async with async_session() as db:
        finalized_task = await db.scalar(
            select(MediaGenerationTask).where(MediaGenerationTask.reservation_id == finalized_reservation_id)
        )
    ...
    async with async_session() as db:
        ...
        failure_notification_count = await db.scalar(
            select(func.count()).select_from(Notification).where(Notification.ref_id == failure_task_id)
        )
    ...
    recovery_raw_key = str((recovery_created.request_metadata or {})["recovery_asset_storage_key"])
    ...
    async with async_session() as db:
        ...
        metadata["raw_capture_deadline_at"] = (datetime.now(timezone.utc) - timedelta(minutes=1)).isoformat()
    ...
    recovery_final = await media_generation.reconcile_minimax_sync_media_task(recovery_task_id)
    ...
    lease_raw_key = str((lease_created.request_metadata or {})["recovery_asset_storage_key"])
    ...
    claim_a_status, claim_a = await media_generation._claim_sync_local_processing(lease_task_id)
    ...
    async with async_session() as db:
        ...
        await db.commit()
    claim_b_status, claim_b = await media_generation._claim_sync_local_processing(lease_task_id)
    ...
    assert storage.objects[f"{sync_agent_id}/workspace/images/lease-fence.png"] == lease_png_b
    ...
    compensation_final = await media_generation.reconcile_minimax_sync_media_task(compensation_task_id)
    ...
    async with async_session() as db:
        ...
        recovery_activity_count = await db.scalar(
            select(func.count()).select_from(AgentActivityLog).where(AgentActivityLog.related_id == recovery_task_id)
        )
        lease_activity_count = await db.scalar(
            select(func.count()).select_from(AgentActivityLog).where(AgentActivityLog.related_id == lease_task_id)
        )
            ...
            MediaGenerationTask(
                ...
                request_metadata={
                    "recovery_asset_storage_key": (
                        f"_internal/provider_recovery/minimax/sync/{sync_agent_id}/{corrupt_task_id}/image.bin"
                    ),
                },
            )
        )
