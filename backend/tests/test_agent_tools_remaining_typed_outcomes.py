...
def test_migrated_tools_are_runtime_visible_only_after_typed_migration() -> None:
    ...
    assert "finish" in {definition["name"] for definition in BUILTIN_TOOL_DEFINITIONS}
    assert default_application_tools - {"finish"} <= (
        agent_tools.RUNTIME_TYPED_APPLICATION_TOOL_NAMES
    )
...
async def test_send_file_to_agent_rejects_parent_traversal_before_storage_access(
    ...
    assert outcome.status == "failed"
    assert outcome.error_code == "invalid_workspace_path"


def test_minimax_typed_lifecycle_facts_do_not_infer_status_from_display_text() -> None:
    agent_id = uuid.uuid4()
    record_id = uuid.uuid4()

    unknown = agent_tools._minimax_tool_result(
        "✅ provider returned presentation text",
        typed=True,
        status="unknown",
        error_code="minimax_submission_unknown",
        agent_id=agent_id,
        modality="video",
        record_id=record_id,
    )
    success = agent_tools._minimax_tool_result(
        "asset persisted",
        typed=True,
        status="succeeded",
        agent_id=agent_id,
        modality="image",
        record_id=record_id,
        output_path="workspace/images/result.png",
    )

    assert isinstance(unknown, ToolExecutionOutcome)
    assert unknown.status == "unknown"
    assert unknown.error_code == "minimax_submission_unknown"
    assert success.status == "succeeded"
    assert success.artifact_refs == (
        f"workspace://{agent_id}/workspace/images/result.png",
    )
