...
async def test_setup_lock_is_released_after_saver_failure(monkeypatch) -> None:
    ...
    assert "pg_try_advisory_lock" in events[1][0]
    assert "pg_advisory_unlock" in events[2][0]
    assert events[-1] == ("closed", None)


@pytest.mark.asyncio
async def test_setup_lock_polls_without_holding_a_waiting_transaction(monkeypatch) -> None:
    events = []
    ...
    monkeypatch.setattr(setup_langgraph_checkpoints.asyncio, "sleep", sleep)

    async with setup_langgraph_checkpoints.checkpoint_setup_lock(
        Settings(DATABASE_URL="postgresql+asyncpg://app:secret@db/clawith")
    ):
        events.append(("locked", None))

    try_lock_events = [
        event for event in events if "pg_try_advisory_lock" in event[0]
    ]
    assert len(try_lock_events) == 2
    sleep.assert_awaited_once_with(0.1)
    assert events[-2][0].startswith("SELECT pg_advisory_unlock")
    assert events[-1] == ("closed", None)
