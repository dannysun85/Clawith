...
def test_parse_quota_observations_separates_general_and_media_models():
    ...
    assert {(observation.modality, observation.model): observation.depleted for observation in observations} == {
        ("plan", None): True,
...
@pytest.mark.asyncio
async def test_poller_requires_recognized_provider_evidence_before_clearing_circuits(
    monkeypatch,
):
    credential = SimpleNamespace(
        id=uuid.uuid4(),
    ...
    monkeypatch.setattr(
        ...
        lambda _credential: "secret",
    )
    monkeypatch.setattr(
        minimax_quota,
        "_check_credential_quota",
    ...
    assert depleted == 0
    assert minimax_quota._quota_cycle_active_credentials == 1
    assert minimax_quota._quota_cycle_provider_evidence == 0
    mark.assert_not_awaited()
    clear.assert_not_awaited()


def test_quota_monitor_health_separates_loop_liveness_from_provider_evidence(
    monkeypatch,
):
    now = datetime(2026, 7, 16, 8, 0, tzinfo=timezone.utc)
    monkeypatch.setattr(minimax_quota, "_quota_monitor_interval_seconds", 300)
    monkeypatch.setattr(
        minimax_quota,
        "_quota_monitor_started_at",
        now - timedelta(minutes=30),
    )
    monkeypatch.setattr(
        minimax_quota,
        "_quota_monitor_last_loop_success_at",
        now - timedelta(minutes=2),
    )
    monkeypatch.setattr(
        minimax_quota,
        "_quota_monitor_last_provider_evidence_at",
        now - timedelta(minutes=21),
    )
    monkeypatch.setattr(minimax_quota, "_quota_cycle_active_credentials", 2)
    monkeypatch.setattr(minimax_quota, "_quota_cycle_provider_evidence", 0)

    health = minimax_quota.minimax_quota_monitor_health(now=now)

    assert health["healthy"] is True
    assert health["provider_evidence_required"] is True
    assert health["provider_evidence_fresh"] is False
    assert health["active_credentials"] == 2
    assert health["credentials_with_provider_evidence"] == 0


def test_quota_monitor_health_does_not_require_external_evidence_without_credentials(
    monkeypatch,
):
    now = datetime(2026, 7, 16, 8, 0, tzinfo=timezone.utc)
    monkeypatch.setattr(minimax_quota, "_quota_monitor_interval_seconds", 300)
    monkeypatch.setattr(minimax_quota, "_quota_monitor_started_at", now)
    monkeypatch.setattr(
        minimax_quota,
        "_quota_monitor_last_loop_success_at",
        now,
    ...
    monkeypatch.setattr(minimax_quota, "_quota_cycle_active_credentials", 0)
    monkeypatch.setattr(minimax_quota, "_quota_cycle_provider_evidence", 0)

    health = minimax_quota.minimax_quota_monitor_health(now=now)

    assert health["healthy"] is True
    assert health["provider_evidence_required"] is False
    assert health["provider_evidence_fresh"] is True


@pytest.mark.asyncio
async def test_quota_monitor_exits_after_repeated_infrastructure_failures(
    monkeypatch,
):
    settings = SimpleNamespace(
        MINIMAX_QUOTA_MONITOR_INTERVAL_SECONDS=30,
        APP_VERSION="1.10.12",
    )
    ...
    monkeypatch.setattr(minimax_quota.asyncio, "sleep", AsyncMock())
    monkeypatch.setattr(
        minimax_quota,
        "MINIMAX_QUOTA_MONITOR_MAX_CONSECUTIVE_FAILURES",
        2,
    )

    with pytest.raises(
        RuntimeError,
        match="MiniMax quota monitor exceeded its failure threshold",
    ):
        await minimax_quota.start_minimax_quota_monitor_daemon()


def test_quota_monitor_is_a_first_class_worker_not_a_nested_lifecycle_task():
    from pathlib import Path

    root = Path(__file__).resolve().parents[2]
    main_source = (root / "backend/app/main.py").read_text()
    lifecycle_source = (root / "backend/app/services/subscription_lifecycle.py").read_text()
    compose_source = (root / "deploy/astra-poc/docker-compose.prod.yml").read_text()

    assert '("minimax_quota_monitor", start_minimax_quota_monitor_daemon())' in main_source
    assert "MINIMAX_QUOTA_MONITOR_INTERVAL_SECONDS" in compose_source
    assert "start_minimax_quota_monitor_daemon" not in lifecycle_source
    assert "poll_minimax_quota" not in lifecycle_source
