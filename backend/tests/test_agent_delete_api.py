...
async def test_delete_agent_marks_deleted_and_preserves_history(monkeypatch):
    ...
    async def no_blockers(_db, checked_agent_id):
        assert checked_agent_id == agent.id
        return []
    ...
    monkeypatch.setattr(agents_api, "_agent_deletion_blockers", no_blockers)
    monkeypatch.setattr(agents_api, "enqueue_cancel", fake_enqueue_cancel)
    ...
    assert agent.deletion_requested_at is not None
...
@pytest.mark.asyncio
async def test_delete_agent_keeps_logical_delete_when_container_removal_fails(
    monkeypatch,
):
    creator = make_user()
    agent = make_agent(creator)
