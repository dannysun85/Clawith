...
async def test_runtime_trigger_pins_execution_identity_and_caller_transaction() -> None:
    ...
    assert command.payload["message_id"] == str(uuid.uuid5(execution.id, "runtime-trigger-input"))
    assert command.payload["input_content"].startswith("===== 本次唤醒上下文 =====")
    assert "trigger_context" not in command.payload
    assert command.payload["trigger_event_data"] == {"webhook_payload": '{"status": "ready"}'}
