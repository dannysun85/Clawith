"""Channel credentials must never leak through the shared response schema.

Regression coverage for upstream f274fd8c ("redact channel credentials from
responses"), adapted to the local status-only ChannelConfigOut contract.
"""
...
def test_channel_config_response_excludes_credentials_in_all_channel_endpoints() -> None:
    ...
    for secret in ("bot-token", "signing-secret", "verification-token", "bot-secret", "access-token"):
