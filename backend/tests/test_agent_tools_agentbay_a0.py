...
async def test_dispatch_keeps_durable_arguments_deeply_unchanged_and_does_not_inject_session_id(
    ...
    # This test isolates the immutable argument boundary. Autonomy policy is
    # covered independently and remains fail-closed in production.
    monkeypatch.delitem(
        agent_tools._TOOL_AUTONOMY_MAP,
        "agentbay_command_exec",
    )
...
@pytest.mark.asyncio
async def test_same_run_reuses_agentbay_session(
    monkeypatch: pytest.MonkeyPatch,
):
...
@pytest.mark.asyncio
async def test_different_runs_get_isolated_agentbay_sessions(
    monkeypatch: pytest.MonkeyPatch,
):
    _install_fake_agentbay_sdk(monkeypatch)
    agent_id = uuid.uuid4()
...
async def test_preview_reuses_only_the_exact_session_and_environment(
    ...
    agentbay_client._agentbay_sessions[(agent_id, session_id, image_type)] = (
        cached,
        datetime.now(),
    )

    async def exact_existing(
        requested_agent_id: uuid.UUID,
        requested_image_type: str,
        requested_session_id: str,
    ):
        assert requested_agent_id == agent_id
        assert requested_image_type == image_type
    ...
    result = await getattr(agentbay_live, reader_name)(agent_id, session_id)
...
async def test_take_control_never_fuzzy_reuses_another_scope_or_environment(
    ...
    assert factory_calls == [(agent_id, "computer", requested_session_id)]
...
async def test_take_control_reuses_an_exact_scoped_environment(
    ...
    cached = _ControlClient("cached")
    agentbay_client._agentbay_sessions[
        (agent_id, requested_session_id, "computer")
    ] = (cached, datetime.now())
    ...
    async def exact_existing(
        ...
        assert (requested_agent_id, image_type, session_id) == (
            agent_id,
            "computer",
            requested_session_id,
        )
