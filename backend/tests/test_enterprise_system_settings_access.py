...
Ported from upstream 27650d57 and adapted to the local authority model:
platform authority comes from ``identity.is_platform_admin`` and company
authority from the active membership role (``org_admin``/``org_owner``).
"""
...
def test_platform_admin_can_manage_global_and_tenant_scoped_settings() -> None:
    platform_admin = _user(role="member", platform_identity=True)
