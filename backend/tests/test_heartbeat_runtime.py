...
async def test_heartbeat_intake_error_does_not_read_expired_agent_identity() -> None:
    ...
    with (
        ...
        patch.object(heartbeat_service.settings, "HEARTBEAT_ENABLED", True),
    ):
...
@pytest.mark.asyncio
async def test_background_runtime_resolves_and_pins_unified_saas_route() -> None:
    agent = _agent()
    agent.preferred_tier = "ultra"
    agent.preferred_modality = "video"
    routed_model_id = uuid.uuid4()
    route_meta = type(
        "RouteMeta",
        (),
        {"saas_tier": "ultra", "modality": "video"},
    )()
    routed_model = type("Model", (), {"id": routed_model_id})()
    handle = RunHandle(
        tenant_id=agent.tenant_id,
        run_id=uuid.uuid4(),
        thread_id=str(uuid.uuid4()),
    ...
    with (
        patch(
            ...
            new=AsyncMock(return_value=(routed_model, None, route_meta)),
        ) as resolve_model,
        patch(
            "app.services.heartbeat_runtime.RuntimeCommandIntake.start_run",
            new=AsyncMock(return_value=handle),
        ...
        result = await enqueue_oneshot_runtime(
            ...
            occurrence_id=uuid.uuid4(),
            triggered_by_user_id=agent.creator_id,
            requested_model_turn_limit=4,
            settings_override=_settings(enabled=True),
        )

    assert result == handle
    resolve_model.assert_awaited_once_with(agent)
    command = start_run.await_args.args[0]
    assert command.model_id == routed_model_id
    assert command.payload["saas_tier"] == "ultra"
    assert command.payload["model_modality"] == "video"
