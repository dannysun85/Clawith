...
async def test_gateway_native_agent_message_commits_runtime_before_acceptance() -> None:
    ...
    with (
        ...
        patch("app.api.gateway._touch_gateway_agent", new=AsyncMock()),
        patch(
            "app.api.gateway.get_agent_access_level_for_user_id",
            new=AsyncMock(return_value="manage"),
        ),
        patch(
    ...
    assert db.commits == 1
    # The hardened gateway closes the authentication transaction before the
    # independent liveness touch, avoiding a reciprocal A2A lock cycle.
...
async def test_openclaw_report_resumes_native_run_in_the_gateway_commit() -> None:
    ...
    with (
        ...
        patch("app.api.gateway._touch_gateway_agent", new=AsyncMock()),
        patch(
            "app.api.gateway._authorize_gateway_message",
            new=AsyncMock(return_value=SimpleNamespace(session=SimpleNamespace(agent_id=target.id))),
        ),
        patch(
