...
async def test_slack_webhook_uses_runtime_intake(monkeypatch) -> None:
    ...
    config = SimpleNamespace(encrypt_key="signing-secret", app_secret="")
    ...
    monkeypatch.setattr(slack, "_verify_slack_signature", lambda *_args: True)
...
async def test_teams_webhook_uses_runtime_intake(monkeypatch) -> None:
    ...
    monkeypatch.setattr(teams.settings, "TEAMS_WEBHOOK_ENABLED", True)
...
@pytest.mark.asyncio
async def test_teams_webhook_rejects_requests_without_a_valid_jwt(monkeypatch) -> None:
    monkeypatch.setattr(teams.settings, "TEAMS_WEBHOOK_ENABLED", True)
    agent_id = uuid.uuid4()
...
async def test_whatsapp_webhook_uses_runtime_intake(monkeypatch) -> None:
    ...
    config = SimpleNamespace(encrypt_key="app-secret", app_id="phone-number-1", app_secret="token-1")
    ...
    monkeypatch.setattr(whatsapp, "_verify_signature", lambda *_args: True)
...
async def test_discord_interaction_commits_runtime_before_deferred_ack(monkeypatch) -> None:
    ...
    config = SimpleNamespace(encrypt_key="public-key", app_id="app-1", app_secret="bot-token-1")
    agent = SimpleNamespace(id=agent_id, tenant_id=tenant_id, creator_id=uuid.uuid4())
    ...
    monkeypatch.setattr(discord_bot, "_verify_discord_signature", lambda *_args: True)
