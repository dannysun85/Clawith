...
@pytest.mark.asyncio
async def test_short_session_skips_model_policy_until_old_messages_exist(
    monkeypatch,
) -> None:
    tenant_id = uuid.uuid4()
    ...
    class _ContextService:
        async def load_snapshot(self, value, **kwargs):
            assert value is db
            assert kwargs == {"tenant_id": tenant_id, "session_id": session_id}
            calls.append("snapshot")
            return SessionContextSnapshot.empty()

        async def load_compactable_messages_after_watermark(self, value, **kwargs):
            assert value is db
            assert kwargs == {
                "tenant_id": tenant_id,
            ...
            calls.append("compactable_messages")
            return ()

        async def load_recent_user_visible_messages(self, *_args, **_kwargs):
            raise AssertionError("recent messages are unnecessary without compactable input")

    class _PolicyResolver:
        async def resolve(self, *_args, **_kwargs):
            raise AssertionError("model policy is unnecessary without compactable input")

    service = background.SessionContextMessageCompactionService(
        lock_engine=object(),  # type: ignore[arg-type]
        compactor=object(),  # type: ignore[arg-type]
        context_service=_ContextService(),  # type: ignore[arg-type]
        policy_resolver=_PolicyResolver(),  # type: ignore[arg-type]
    )
    monkeypatch.setattr(background, "AsyncSession", _AsyncSessionContext)

    assert await service._load_request(  # type: ignore[arg-type]
        object(),
        tenant_id=tenant_id,
        session_id=session_id,
    ) is None
    assert calls == ["snapshot", "compactable_messages"]
