...
async def checkpoint_setup_lock(
    ...
    try:
        # Do not block inside ``pg_advisory_lock``. A waiting PostgreSQL query
        # keeps a virtual transaction open, while the lock holder's LangGraph
        # setup may execute ``CREATE INDEX CONCURRENTLY`` and wait for that same
        # virtual transaction. Polling with try-lock ends every unsuccessful
        # statement before the saver opens its DDL connection and avoids that
        # cross-session deadlock.
        while True:
