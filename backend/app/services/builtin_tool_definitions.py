...
_BUILTIN_TOOL_SOURCE = [
    ...
    },
    {
        ...
        "parameters_schema": {
            ...
            "properties": {
                ...
                "page_start": {"type": "integer", "minimum": 1, "description": "First PDF page or PPT slide to read (1-based, default 1)"},
                "max_pages": {"type": "integer", "minimum": 1, "maximum": 50, "description": "Maximum PDF pages or PPT slides to read (default 50)"},
                "max_chars": {"type": "integer", "minimum": 1, "maximum": 20000, "description": "Maximum extracted characters (default 8000)"}
            },
            "required": ["path"],
        },
        "config": {},
    ...
    },
    {
        ...
        "parameters_schema": {
            ...
            "properties": {
                ...
                "slide_spec_path": {"type": "string", "description": "Optional workspace JSON page specification. Must be supplied together with outline_path; slide ids and headlines are validated against the HTML."},
            },
            "required": ["source_path", "target_path"],
        },
        "config": {},
    ...
    },
    {
        ...
        "description": "Send a private A2A message to a digital employee from query_directory. notify completes after the durable send. consult and task_delegate create a delegated Run; this Run waits and resumes with the correlated result, so do not poll or send the same request again. CEO coordinator task_delegate calls must include a delegation_contract whose required capabilities match real target tool grants.",
        "category": "communication",
        "icon": "🤖",
        "is_default": True,
        "parameters_schema": {
            ...
            "properties": {
                ...
                "delegation_contract": {
                    "type": "object",
                    "description": "Structured, server-bound contract for verifiable task_delegate work. Required for CEO coordinator delegation.",
                    ...
                    "properties": {
                        ...
                        "requires_artifact_delivery": {"type": "boolean"},
                    },
                    "required": [
                        "version",
                        "title",
...
# v1.10.13 shipped five MiniMax media tools and three Plaza tools before the
# upstream canonical registry existed.  Import only names that upstream does
# not own; shared names always keep the reviewed v1.11 contract.  This bridge
# also lets the canonical seed export replace the old duplicated seeder list
# below, so the database and model schemas cannot drift again.
from app.services import tool_seeder as _legacy_tool_seeder  # noqa: E402

_canonical_source_names = {str(seed["name"]) for seed in _BUILTIN_TOOL_SOURCE}
_legacy_seed_by_name = {
    str(seed["name"]): seed for seed in _legacy_tool_seeder.BUILTIN_TOOLS
}
_BUILTIN_TOOL_SOURCE = [
    *_BUILTIN_TOOL_SOURCE,
    *(
        deepcopy(seed)
        for name, seed in _legacy_seed_by_name.items()
        if name not in _canonical_source_names
    ),
]

# Preserve the local deterministic copy/product overlay fields on every image
# generator while keeping all other upstream schema decisions authoritative.
_brand_safe_image_names = {
...
for _seed in _BUILTIN_TOOL_SOURCE:
    _name = str(_seed["name"])
    if _name not in _brand_safe_image_names:
        continue
    _legacy = _legacy_seed_by_name.get(_name)
    if _legacy is None:
        continue
    _properties = _seed["parameters_schema"].setdefault("properties", {})
    _legacy_properties = _legacy["parameters_schema"].get("properties", {})
...
# Compatibility export: all startup seeding now consumes the same canonical
# immutable tuple even while the old file is retained during this upgrade.
_legacy_tool_seeder.BUILTIN_TOOLS = BUILTIN_TOOL_SEEDS
_legacy_tool_seeder.BUILTIN_TOOL_NAMES = frozenset(
    seed["name"] for seed in BUILTIN_TOOL_SEEDS
)
