...
from app.services.llm.caller import prepare_pinned_agent_llm_invocation
    ...
    async def _call_prepared(
        ...
        *,
        ...
        if self._completion is complete_llm_once:
            initial_input = state["snapshots"].initial_input
            raw_user_id = context.actor_user_id or initial_input.get("user_id")
            try:
                user_id = uuid.UUID(str(raw_user_id)) if raw_user_id else None
            except (TypeError, ValueError, AttributeError):
                user_id = None
            tier = initial_input.get("saas_tier")
            modality = initial_input.get("model_modality")
            invocation = await prepare_pinned_agent_llm_invocation(
                agent,
                model,
                tier=tier if isinstance(tier, str) and tier else None,
                modality=(
                    modality if isinstance(modality, str) and modality else None
                ),
                action=(
                    "heartbeat"
                    if context.source_type == "heartbeat"
                    else "trigger"
                    if context.source_type == "trigger"
                    else "chat"
                ),
            )
