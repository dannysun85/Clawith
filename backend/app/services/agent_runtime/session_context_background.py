    ...
    async def _load_request(
        ...
        *,
        ...
        async with AsyncSession(bind=connection, expire_on_commit=False) as db:
            ...
            # The recent raw-message window is intentionally never compacted.
            # Avoid resolving Agent model budgets until there is actually an
            # older message to compact: short sessions must not depend on model
            # credentials and must not create a retrying background error loop.
            if not messages:
                return None
