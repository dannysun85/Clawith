...
def onboarding_source_execution_id(
    ...
    *,
    ...
    return f"{_ONBOARDING_SOURCE_PREFIX}:{tenant_id}:{agent_id}:{user_id}:{attempt}"
...
def stored_user_content(
    ...
    *,
    ...
    """Persist a display-safe message while keeping inline media transport-only."""
    return sanitize_inline_media_content(
        content,
...
def _chat_goal(
    ...
    if "[image_data:" in visible or "[video_data:" in visible:
        visible = "Analyze the attached media and respond to the user."
    file_label = ", ".join(file_name) if isinstance(file_name, list) else file_name
    if file_label:
        visible = f"{visible}\nAttached file: {file_label}" if visible else f"Handle attached file: {file_label}"
...
async def _require_resume_run(
    ...
    *,
    ...
    if (
        ...
        or (direct_thread_id is not None and run.scheduling_lane_key != _direct_lane_key(tenant_id, session_id))
    ):
        raise ChatRuntimeIntakeError(
...
async def enqueue_chat_runtime(
    ...
    *,
    ...
    normalized_saas_tier = saas_tier.strip().lower() if isinstance(saas_tier, str) else ""
    normalized_model_modality = model_modality.strip().lower() if isinstance(model_modality, str) else ""
    ...
    normalized_source_execution_id = (
        source_execution_id_override.strip() if isinstance(source_execution_id_override, str) else ""
    )
    ...
    if resume_run_id is not None:
        resumed_run = await _require_resume_run(
            ...
            user_id=user.id,
            direct_thread_id=(str(session.id) if session.session_type == "direct" else None),
        )
    ...
    handle = await adapter.start_run(
        StartRunCommand(
            ...
            scheduling_lane_key=(_direct_lane_key(tenant_id, session.id) if is_direct_thread else None),
            scheduling_position_created_at=(scheduling_position_created_at if is_direct_thread else None),
