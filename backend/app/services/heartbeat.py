...
async def _heartbeat_tick():
    ...
    # The operator kill switch must be evaluated before trace allocation or a
    # database session is opened.  This keeps disabled CEO/agent heartbeats
    # genuinely idle instead of scanning every tenant once per minute.
        ...
        async with async_session() as db:
            ...
            for agent in agents:
                ...
                if not runtime_decision.use_v2:
                    logger.error(
                        "Heartbeat remains due agent_id={} Runtime disabled reason={}",
                        agent.id,
            ...
            if triggered:
                try:
                    ...
                    logger.warning(
                        "Failed to write heartbeat_tick audit error_type={}",
                        type(exc).__name__,
                    )

    except Exception as exc:
        error_type = type(exc).__name__
        logger.exception("Heartbeat tick error error_type={}", error_type)
        await write_audit_log("heartbeat_error", {"error_type": error_type})
...
async def _notify_oneshot_error(
    ...
    try:
        ...
        async with async_session() as db:
            ...
            await db.commit()
        logger.info(
            "[Oneshot] Failure notification created user_id={} agent_id={}",
    ...
    try:
        ...
        async with async_session() as db:
            ...
            if handle is None:
                ...
                logger.error(
                    "[Oneshot] Runtime unavailable agent_id={} message_chars={}",
                    agent_id,
                    len(message),
                )
                return ""
            await db.commit()
        logger.info(
            "[Oneshot] Queued run_id={} agent_id={}",
            handle.run_id,
            agent_id,
        )
        ...
        logger.error(
            "[Oneshot] Runtime intake failed agent_id={} code={} error_type={}",
            agent_id,
            exc.code,
            type(exc).__name__,
        )
        ...
        logger.exception(
            "[Oneshot] Unexpected error agent_id={} error_type={}",
            agent_id,
            type(exc).__name__,
        )
        return ""
