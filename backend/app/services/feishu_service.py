...
from app.services.auth_provider import ExternalUserInfo, FeishuAuthProvider
...
class FeishuService:
    ...
    def _parse_api_response(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "[Feishu] {} returned non-JSON response http_status={} "
                "message_id_present={} error_type={}",
                stage,
                resp.status_code,
                bool(message_id),
                type(e).__name__,
        ...
        if not 200 <= resp.status_code < 300:
            logger.warning(
                "[Feishu] {} HTTP failure http_status={} message_id_present={} response_fields={}",
                stage,
                resp.status_code,
                bool(message_id),
        ...
        token_response_without_code = (
            code is None
            and isinstance(data, dict)
            and bool(
                data.get("tenant_access_token")
                or data.get("app_access_token")
            )
            and stage.endswith("_token")
        )
        if code != 0 and not token_response_without_code:
            logger.warning(
                "[Feishu] {} business failure message_id_present={} code={}",
                stage,
                bool(message_id),
                code,
            )
    ...
    async def login_or_register(
        ...
        """
        # Resolve the provider record so stable subjects stay scoped to the
        # configured Feishu application and tenant.
        ...
        user_info = ExternalUserInfo(
            provider_type="feishu",
            provider_union_id=feishu_user.get("union_id"),
            provider_user_id=feishu_user.get("user_id") or feishu_user.get("open_id"),
            name=feishu_user.get("name", ""),
            email=feishu_user.get("email", ""),
            avatar_url=feishu_user.get("avatar_url", ""),
            mobile=feishu_user.get("mobile", ""),
            raw_data=feishu_user,
        )
        auth_provider = FeishuAuthProvider(
            provider=provider,
            config={"app_id": self.app_id, "app_secret": self.app_secret},
        )
        ...
        if not external_user_can_authenticate(user):
            raise ValueError("Feishu account is disabled")
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            if accompany_msg:
                ...
                if text_resp.status_code != 200:
                    logger.error(
                        "[Feishu] Failed to send text accompany message status={} "
                        "response_chars={} receive_id_type={}",
    ...
    def _get_lark_client(self, app_id: str, app_secret: str):
        ...
        if client is None:
            ...
            if len(self._lark_clients) >= self._LARK_CLIENT_CACHE_MAX:
                ...
                logger.debug("[Feishu] _lark_clients LRU evict")
    ...
    async def create_card_entity(
        ...
        try:
            ...
            logger.info(
                "[Feishu CardKit] create_card_entity response code={} success={}",
                resp.code,
                resp.success(),
            )
            if not resp.success():
    ...
    async def stream_card_content(
        ...
        try:
            ...
            logger.info(
                "[Feishu CardKit] stream_card_content response code={} success={} sequence={}",
                resp.code,
                resp.success(),
                sequence,
            )
    ...
    async def set_card_streaming_mode(
        ...
        try:
            ...
            logger.info(
                "[Feishu CardKit] set_card_streaming_mode response code={} success={} "
                "streaming_mode={} sequence={}",
                resp.code,
                resp.success(),
                streaming_mode,
                sequence,
    ...
    async def update_cardkit_card(
        ...
        try:
            ...
            logger.info(
                "[Feishu CardKit] update_cardkit_card response code={} success={} sequence={}",
                resp.code,
                resp.success(),
                sequence,
            )
