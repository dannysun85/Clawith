...
async def enqueue_trigger_execution(
    ...
    *,
    ...
    """Atomically register one Trigger occurrence and its Runtime command."""
    ...
    fire_count = stored_trigger.fire_count or 0
    at_capacity = (
        stored_trigger.type == "once" and fire_count >= 1
    ) or (
        bool(stored_trigger.max_fires)
        and fire_count >= stored_trigger.max_fires
    )
    if not stored_trigger.is_enabled or at_capacity:
        _fail_runtime_execution(
            execution,
            TriggerRuntimeIntakeError(
                "trigger_disabled" if not stored_trigger.is_enabled else "trigger_capacity_exhausted",
                "Trigger was not eligible when its execution was accepted",
            ),
            now,
        )
        if at_capacity:
    ...
    if agent is None:
        ...
        try:
            async with db.begin_nested():
                ...
                execution.fire_recorded_at = now
                await db.flush()
