...
def _trigger_event_data(trigger: AgentTrigger) -> dict[str, str]:
    ...
    if trigger.type == "webhook" and config.get("_webhook_payload"):
        ...
        event_data["webhook_payload"] = payload if len(payload) <= 2_000 else payload[:2_000] + "... (truncated)"
    return event_data
...
def build_trigger_context(triggers: list[AgentTrigger]) -> str:
    ...
    for trigger in triggers:
        ...
        if trigger.name == "daily_okr_collection":
            ...
            part += (
                "\n执行要求：本次公司级报表由系统自动汇总生成。如果你被唤醒，仅补充必要说明，不要再次向成员发起收集。"
            )
        ...
        config = _trigger_config(trigger)
        if trigger.type == "on_message" and config.get("okr_member_id") and config.get("okr_report_date"):
            part += (
                "\n执行要求：这是一次日报回复入库事件。"
...
async def _ensure_trigger_session(
    ...
    *,
    ...
    if session is None:
        ...
        db.add(session)
    elif session.tenant_id != agent.tenant_id or session.session_type != "trigger" or session.agent_id != agent.id:
        raise TriggerRuntimeIntakeError(
...
async def enqueue_trigger_runtime(
    ...
    *,
    ...
    origin_user_id = uuid.UUID(delivery_target["user_id"]) if delivery_target is not None else agent.creator_id
    execution_id = str(execution.id)
    scheduling_position = execution.scheduled_at or execution.created_at or datetime.now(UTC)
