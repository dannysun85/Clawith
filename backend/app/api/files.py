...
def _canonical_agent_file_path(path: str) -> str:
    """Use the write layer's canonical path for policy and storage decisions."""
    return normalize_workspace_path((path or "").strip()).strip("/")


def _is_internal_skill_metadata_path(path: str) -> bool:
    """Return whether a workspace path points at product-owned Skill metadata."""
    normalized = _canonical_agent_file_path(path)
...
def _requires_agent_file_manage_access(path: str) -> bool:
    """Return whether a workspace path is part of the Agent control plane.

    Soul, durable Memory, and Skills can change an Agent's future behavior. They
    therefore share the same manager-only mutation and revision boundary even
    though ordinary workspace artifacts remain available to ``use`` members.
    """
    normalized = PurePosixPath(_canonical_agent_file_path(path)).as_posix().lower()
    return (
        normalized in {"soul.md", "memory.md", "memory", "skills"}
...
def _visible_path(agent_id: uuid.UUID, rel_path: str, tenant_id: uuid.UUID | None) -> tuple[Path, Path, bool]:
    ...
    try:
        resolved = resolve_agent_visible_path(
            ...
            _canonical_agent_file_path(rel_path),
...
def _is_enterprise_visible_path(rel_path: str) -> bool:
    normalized = _canonical_agent_file_path(rel_path)
...
async def list_files(
    ...
    canonical_path = _canonical_agent_file_path(path)
    storage_key, is_enterprise = _visible_storage_key(agent_id, canonical_path, current_user.tenant_id)
...
async def read_file(
    ...
    path = _canonical_agent_file_path(path)
...
async def write_file(
    ...
    path = _canonical_agent_file_path(path)
    required_level = "manage" if _requires_agent_file_manage_access(path) else "use"
    await check_agent_access(
        db,
        current_user,
        agent_id,
...
async def lock_file(
    ...
    path = _canonical_agent_file_path(data.path)
    required_level = "manage" if _requires_agent_file_manage_access(path) else "use"
    await check_agent_access(
        db,
        current_user,
...
async def restore_file_revision(
    ...
    revision_path = _canonical_agent_file_path(revision.path)
    if _requires_agent_file_manage_access(revision_path):
        await check_agent_access(
            db,
            current_user,
            agent_id,
    ...
    _reject_internal_skill_metadata_path(revision_path, mutation=True)
    ...
    await db.commit()
    return {"status": "ok", "path": revision_path, "revision_id": restored.revision_id}
...
async def upload_file_to_workspace(
    ...
    normalized_path = _canonical_agent_file_path(path)
