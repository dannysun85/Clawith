...
RUNTIME_UNIQUES = {
    ...
    "agent_run_commands": {"uq_agent_run_commands_run_idempotency": ("run_id", "idempotency_key")},
    "agent_run_events": {
        "uq_agent_run_events_run_idempotency": ("run_id", "idempotency_key"),
    },
    "agent_tool_executions": {"uq_agent_tool_executions_run_tool_call": ("run_id", "tool_call_id")},
    "session_context_states": {"uq_session_context_states_session_id": ("session_id",)},
}
RUNTIME_CHECKS = {
    "agent_runs": {
        "ck_agent_runs_source_type": ("source_type IN ('chat', 'trigger', 'task', 'a2a', 'heartbeat')"),
        "ck_agent_runs_run_kind": ("run_kind IN ('foreground', 'background', 'delegated', 'orchestration')"),
        "ck_agent_runs_runtime_type": "runtime_type IN ('legacy', 'langgraph')",
        "ck_agent_runs_delivery_status": ("delivery_status IN ('not_required', 'pending', 'delivered', 'failed')"),
    ...
    "agent_tool_executions": {
        ...
        "ck_agent_tool_executions_effect": ("effect IN ('read', 'write', 'external_write')"),
        "ck_agent_tool_executions_retry_policy": ("retry_policy IN ('safe', 'conditional', 'never')"),
        "ck_agent_tool_executions_attempt_count": "attempt_count >= 1",
    },
    "session_context_states": {"ck_session_context_states_version": "version >= 1"},
}
RUNTIME_INDEXES = {
...
BASELINE_ORM_INDEXES = {
    "gateway_messages": ("ix_gateway_messages_delivery_claim",),
    "notifications": (
...
_BASELINE_ORM_TABLE_OBJECTS = {
    table_name: {
        ...
        *(f"constraint:{name}" for name in BASELINE_ORM_CONSTRAINTS[table_name]),
    }
...
_NOTIFICATION_CORE_OBJECTS = _BASELINE_ORM_TABLE_OBJECTS["notifications"] - {
    f"index:{name}" for name in BASELINE_ORM_INDEXES["notifications"]
}
_NOTIFICATION_LEGACY_CORE_OBJECTS = _NOTIFICATION_CORE_OBJECTS - _NOTIFICATION_EXTENSION_OBJECTS
...
_PRECREATED_PHASE_OBJECTS = {
    ...
    "runtime_schema": {
        ...
        *(f"constraint:{name}" for checks in RUNTIME_CHECKS.values() for name in checks),
        *(f"constraint:{name}" for uniques in RUNTIME_UNIQUES.values() for name in uniques),
    },
}
...
# Only these non-destructive, final-shape indexes are safe to repair in place;
# missing columns or constraints must remain a hard failure.
_PRECREATED_REPAIRABLE_OBJECTS = {
    "unified_chat": {
        "index:ix_chat_sessions_tenant_id",
        "index:ix_chat_sessions_group_id",
        "index:uq_chat_sessions_primary_group",
...
UNIFIED_CHAT_DOWNGRADE_SQL = f"session_type IS DISTINCT FROM ({_DERIVED_SESSION_TYPE})"
    ...
    *,
    ...
    for table_name in table_names:
        ...
        if result.first() is not None:
            raise RuntimeError(f"Refusing to downgrade {domain} because {table_name} contains data")
...
def _precreated_phase_state(phase: str, actual: set[str]) -> bool:
    ...
    if set(missing).issubset(repairable):
        return True
    raise RuntimeError(f"Refusing partially precreated {phase} schema; missing: {', '.join(missing)}")
...
def _baseline_orm_table_plan(
    ...
    if table_name == "gateway_messages":
        ...
        if _NOTIFICATION_LEGACY_CORE_OBJECTS.issubset(present) and not _NOTIFICATION_EXTENSION_OBJECTS.intersection(
            present
        ):
...
def _finish_precreated_phase(phase: str, bind: sa.Connection) -> None:
    ...
    if phase == "unified_chat":
        ...
        bind.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_chat_sessions_tenant_id ON chat_sessions (tenant_id)"))
        bind.execute(sa.text("CREATE INDEX IF NOT EXISTS ix_chat_sessions_group_id ON chat_sessions (group_id)"))
        bind.execute(
            sa.text(
                "CREATE UNIQUE INDEX IF NOT EXISTS uq_chat_sessions_primary_group "
                "ON chat_sessions (group_id) "
                "WHERE session_type = 'group' AND group_id IS NOT NULL "
...
_DIRECTORY_INDEX_SQL = (
    "CREATE INDEX ix_agents_tenant_access_status_name ON agents (tenant_id, access_mode, status, name)",
    "CREATE INDEX ix_agents_tenant_creator_access ON agents (tenant_id, creator_id, access_mode)",
    ...
    "CREATE INDEX ix_agent_permissions_scopeid_scope_agent ON agent_permissions (scope_id, scope_type, agent_id)",
    "CREATE INDEX ix_agent_agent_relationships_agent_target ON agent_agent_relationships (agent_id, target_agent_id)",
    "CREATE INDEX ix_org_members_tenant_status_name ON org_members (tenant_id, status, name)",
    "CREATE INDEX ix_org_members_tenant_user ON org_members (tenant_id, user_id)",
)
...
def _create_missing_baseline_index(table_name: str, index_name: str) -> None:
    columns = {
        ...
        "uq_trigger_executions_processing_agent": ("agent_id",),
    }[index_name]
    ...
    elif index_name == "uq_trigger_executions_processing_agent":
        kwargs["postgresql_where"] = sa.text("status = 'processing' AND lease_owner IS NOT NULL")
    op.create_index(
        index_name,
        table_name,
        list(columns),
        unique=index_name == "uq_trigger_executions_processing_agent",
        **kwargs,
    )
...
def _upgrade_baseline_orm_tables() -> None:
    ...
    plans = {table_name: _baseline_orm_table_plan(table_name, actual) for table_name in BASELINE_ORM_TABLES}
    for table_name in BASELINE_ORM_TABLES:
...
def _upgrade_unified_chat() -> None:
    ...
    op.create_index(
        ...
        unique=True,
        postgresql_where=sa.text("session_type = 'direct' AND is_primary = true AND deleted_at IS NULL"),
    )
    op.create_index(
        ...
        postgresql_where=sa.text(
            "session_type = 'group' AND group_id IS NOT NULL AND is_primary = true AND deleted_at IS NULL"
        ),
    )
...
_LLM_CAPABILITY_COLUMNS = {
    "context_window_tokens": lambda: sa.Column("context_window_tokens", sa.Integer(), nullable=True),
    "context_window_tokens_override": lambda: sa.Column("context_window_tokens_override", sa.Integer(), nullable=True),
    "max_input_tokens": lambda: sa.Column("max_input_tokens", sa.Integer(), nullable=True),
    "max_input_tokens_override": lambda: sa.Column("max_input_tokens_override", sa.Integer(), nullable=True),
    "capability_source": lambda: sa.Column("capability_source", sa.String(length=32), nullable=True),
    "capability_checked_at": lambda: sa.Column("capability_checked_at", sa.DateTime(timezone=True), nullable=True),
    "supports_tool_calling": lambda: sa.Column("supports_tool_calling", sa.Boolean(), nullable=True),
    "tool_calling_capability_source": lambda: sa.Column(
        "tool_calling_capability_source", sa.String(length=32), nullable=True
    ),
    "tool_calling_checked_at": lambda: sa.Column("tool_calling_checked_at", sa.DateTime(timezone=True), nullable=True),
    "tool_calling_error": lambda: sa.Column("tool_calling_error", sa.String(length=500), nullable=True),
}
_LLM_CAPABILITY_CHECKS = {
    "ck_llm_models_context_window_tokens_positive": ("context_window_tokens IS NULL OR context_window_tokens > 0"),
    ...
    "ck_llm_models_max_input_tokens_positive": ("max_input_tokens IS NULL OR max_input_tokens > 0"),
    ...
    "ck_llm_models_tool_calling_capability_source": (
        "tool_calling_capability_source IS NULL OR tool_calling_capability_source IN ('probe', 'builtin_registry')"
    ),
}
...
def _downgrade_llm_capabilities() -> None:
    ...
    predicate = " OR ".join(f"{name} IS NOT NULL" for name in _LLM_CAPABILITY_COLUMNS)
    _require_zero(
        bind,
...
def _checks(table_name: str) -> list[sa.CheckConstraint]:
    return [sa.CheckConstraint(expression, name=name) for name, expression in RUNTIME_CHECKS[table_name].items()]
...
def _downgrade_group_workspace_scope() -> None:
    ...
    op.execute(sa.text("DELETE FROM workspace_file_revisions WHERE scope_type = 'group'"))
    op.drop_index(
        "ix_workspace_file_revisions_scope_path",
