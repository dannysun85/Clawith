...
- 状态：`local_candidate_verified`
- 事实来源：`docs/product-line/01-06`、`DESIGN.md`、当前模型/API/前端路由和测试
- 非完成声明：旧候选 `1276da37` 已因角色入口和迁移兼容问题失效；修复后的实现已固化为 `cc6affe7aa1ad35f5bc1e4be0ab4a7247067b248`，PostgreSQL migration smoke、多角色工作树浏览器矩阵、完整自动化门禁和该实现候选的重启/release-identity 复验已有证据。独立代码复审为 `APPROVE / 0 issues`，独立架构复审为 `APPROVE / CLEAR`。真实 Provider、豆包 Benchmark、发布和生产验证未执行。
...
4. **已完成独立收口批次**：反冗余清理、代码审查、架构审查、完整门禁、实现提交和 SHA 绑定管理员浏览器复验均已完成；本文件随最终证据提交固化。
